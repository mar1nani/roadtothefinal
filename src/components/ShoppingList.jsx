...
export default function ShoppingList({ list, onAddItem, onToggleItem, onDeleteItem, onDeleteList }) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 15, color: C.cream, fontWeight: 700 }}>{list.name}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 12, color: C.dim }}>{remaining} restant{remaining > 1 ? "s" : ""}</span>
      ...
      <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
        {list.items.map((it) => (
          <div key={it.id} style={{ display: "flex", alignItems: "center", gap: 8, background: it.checked ? "#0e1838" : "transparent", borderRadius: 8, padding: "6px 8px" }}>
            <div
        ...
        {list.items.length === 0 && <div style={{ fontSize: 12, color: C.dim, textAlign: "center", padding: "6px 0" }}>Liste vide</div>}
      </div>
      ...
      <form onSubmit={submitItem} style={{ display: "flex", gap: 6 }}>
        ...
        />
        <button type="submit" style={{ background: C.accent, border: `1px solid ${C.line}`, borderRadius: 8, padding: "8px 12px", color: C.gold, cursor: "pointer", fontWeight: 700 }}>
          +
        </button>
      </form>
    </div>
