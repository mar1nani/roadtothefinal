        ...
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            <div style={{ fontSize: 15, color: C.cream, fontWeight: 700 }}>
              {[location.city, location.region, location.country].filter(Boolean).join(", ") || "Position GPS"}
            </div>
            {location.lat != null && <WeatherPill lat={location.lat} lon={location.lon} />}
          </div>
          {status === "fallback" && (
            <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>≈ approximatif (étape la plus proche)</div>
          )}
        </>
      ) : (
